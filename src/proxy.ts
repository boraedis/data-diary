...
export function proxy(request: NextRequest) {
  ...
  // src/lib/public-profile.ts); "/public-charts/" is a prefix rather than
  // an exact path since #84 gave each curated chart type its own route
  // (/public-charts/weight etc.) under the nav index at /public-charts
  // itself. Every other route, including the rest of "/api/", is
  // unaffected by this and still requires a session exactly as before.
  const isPublic =
    PUBLIC_PATHS.has(pathname) ||
    pathname.startsWith("/public-charts/") ||
