...
function AddSubcategoryModal({
  ...
  open: boolean;
  onClose: () => void;
  onCreated: (subcategory: GameSubcategoryItem) => void;
}) {
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/game-categories/${categoryId}/subcategories`, {
        method: "POST",
      ...
      onCreated(body as GameSubcategoryItem);
      reset();
      onClose();
    } catch {
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="space-y-1.5">
          <Label htmlFor="add-game-subcategory-name">Name</Label>
          <Input id="add-game-subcategory-name" value={name} onChange={(e) => setName(e.target.value)} autoFocus />
        </div>
...
function SubcategoryRow({
  ...
  onUpdated: (subcategory: GameSubcategoryItem) => void;
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/game-subcategories/${subcategory.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name.trim() }),
      });
      ...
      const updated = { ...subcategory, ...(body as GameSubcategoryItem) };
    ...
    <div className="flex items-center justify-between gap-2 rounded-lg border border-border px-3 py-2">
      ...
      <div className="flex shrink-0 gap-1">
        ...
        <DeleteCatalogItem
          ...
          isBlocked={subcategory.usage.gameCount > 0}
          onDelete={async () => {
            const res = await fetch(`/api/game-subcategories/${subcategory.id}`, { method: "DELETE" });
            if (!res.ok) throw new Error("Failed to delete");
            onDeleted(subcategory.id);
          }}
          blockedContent={
            <p>
              {subcategory.usage.gameCount} game{subcategory.usage.gameCount === 1 ? "" : "s"} still{" "}
              {subcategory.usage.gameCount === 1 ? "carries" : "carry"} this subcategory.
            </p>
          }
        />
      </div>
...
export function GameCategoryDetail({
  category: initial,
  usage,
  subcategories: initialSubcategories,
}: {
  category: GameCategoryItem;
  usage: GameCategoryUsage;
  ...
  async function handleSave() {
    ...
    try {
      ...
      setCategory(body as GameCategoryItem);
      setEditing(false);
      router.refresh();
    } catch {
      setError("Network error");
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <Link href="/manage/entertainment/games/categories" className={buttonVariants({ variant: "outline", size: "sm" })}>
          &larr; Categories
        </Link>
      </div>
      ...
      <Card size="sm">
        ...
        <CardContent className="flex flex-col gap-3">
          {editing ? (
            <>
              <div className="space-y-1.5">
                <Label htmlFor="game-category-name">Name</Label>
                <Input id="game-category-name" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
              <p className="text-xs text-muted-foreground">
                Renaming doesn&rsquo;t update games that already carry the old name — see any game&rsquo;s own
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <DeleteCatalogItem
                  ...
                  isBlocked={usage.gameCount > 0 || usage.subcategoryCount > 0}
                  afterDeleteHref="/manage/entertainment/games/categories"
                  onDelete={async () => {
                    const res = await fetch(`/api/game-categories/${category.id}`, { method: "DELETE" });
                    if (!res.ok) throw new Error("Failed to delete");
                  }}
                  blockedContent={
                    <p>
                      {usage.gameCount} game{usage.gameCount === 1 ? "" : "s"} and {usage.subcategoryCount}{" "}
      ...
      <AddSubcategoryModal
        ...
        onCreated={(subcategory) =>
          setSubcategories((prev) => [...prev, { ...subcategory, usage: { gameCount: 0 } }])
        }
      />
    </>
  );
