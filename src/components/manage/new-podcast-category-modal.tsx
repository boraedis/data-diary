...
export function NewPodcastCategoryModal({
  open,
  onClose,
  onCreated,
}: {
  open: boolean;
  onClose: () => void;
  onCreated: (category: PodcastCategoryItem & { showCount: number }) => void;
}) {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      onCreated({ ...(body as PodcastCategoryItem), showCount: 0 });
      reset();
      onClose();
    } catch {
    ...
    >
      <div className="flex flex-col gap-3">
        <div className="space-y-1.5">
          <Label htmlFor="new-podcast-category-name">Name</Label>
          <Input
