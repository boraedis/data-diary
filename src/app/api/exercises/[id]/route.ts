...
import {
  deleteExerciseCatalogEntry,
  getExerciseCatalogEntry,
  getExerciseUsage,
  updateExerciseCatalogEntry,
  validateExerciseCatalogEntry,
} from "@/lib/days";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await getExerciseCatalogEntry(id);
  if (!item) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const updated = await updateExerciseCatalogEntry(id, parsed.value.name, parsed.value.category);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await deleteExerciseCatalogEntry(id);
